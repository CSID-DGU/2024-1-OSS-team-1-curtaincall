...
from .pic_group import pic_group
...
#
#         metadata_time = get_metadata_time(single_photo)
...
def getsim(stage_id):
    ...
    for filename in all_photo_urls:
        ...
        no_way_home = True
        for group in groups:
            if group.checkin(single_photo, filename):
                no_way_home = False
                break

        if no_way_home:
            new_group = pic_group()
            new_group.create_new(single_photo, filename)
            groups.append(new_group)
            print(groups)


    # make group_to_image as dictionary for groups idx as a key, and list of image urls as a value
    for group in groups:
        group_to_image.append(group.url)
        print(group)
